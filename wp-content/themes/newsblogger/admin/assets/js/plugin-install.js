jQuery(document).ready(function($) {
    'use strict';
    var this_obj = newsblogger_companion_install;
    ...
    $(document).on('click', '.spicethemes-plugin-activate', function(event) {
        ...
        var newURL2= newURL1 + "/admin.php?page=newsblogger-welcome";
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'newsblogger_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.spicethemes-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'newsblogger_hide_customizer_notice';
            sdata.newsblogger_plugin_slug = slug;
        }
        else {
            sdata.action = 'newsblogger_hide_customizer_companion_notice';
        }
